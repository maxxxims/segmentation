...
ANNOTATION_COLOR_CLASS_1 = (0, 255, 0, 0)
ANNOTATION_COLOR_CLASS_0 = (255, 0, 0, 0)
...
def draw_polygons_on_last_figure(last_figure: list, original_img: np.ndarray, 
                                 marker_class_1: list, reverse: bool, alpha: float, ):
...
def draw_pathes(_img: np.ndarray, data: list[dict], reverse: bool = False, alpha: float = 0.8, return_annotation_info: bool = False):
    """_summary_
    ...
    for i, cnt in enumerate(contour):
        ##cv2.drawContours(stencil, cnt, contourIdx=-1, color=MASK_VALUE,thickness=cv2.FILLED) #cv2.NONE_POLISHER
        ...
        if data[i]["selected_class"] == 1:
            ...
            print('ERRRRRORRR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    ##### ANNOTATION_INFO
    if return_annotation_info:
        annotation_info = {
            'class_1': np.sum(img == ANNOTATION_COLOR_CLASS_1),
            'class_0': np.sum(img == ANNOTATION_COLOR_CLASS_0)
        }
    ...
    if return_annotation_info:
        return img_add, img, annotation_info
